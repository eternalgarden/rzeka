...
namespace RzekaRiver
{
    ...
    public class Rzeka : MonoBehaviour, IObservableStream
    {
        ...
        internal Dictionary<Type, X<BB>> _looms = new Dictionary<Type, X<BB>>();

        public void Pluck<T,M>(M matter)
            where T : Thought<M>
            where M : Matter
        {
            _looms.Add(typeof(T), new SomeThought());
        }

        public IWeaveable<T, Min, Mout> Weave<T, Min, Mout>(Min matter, object who)
        ...
        public IWeaveable<T> Weave<T>(object who) where T : ThoughtBase
        {
